        ...
        stage('Validate Hash') {
            steps {
                script {
                    ...
                    def calculatedModelHash = sh(script: "sha256sum ${PWD}/model_weights.json | cut -d' ' -f1", returnStdout: true).trim()
                    def calculatedOutputHash = sh(script: "sha256sum ${PWD}/control_output.txt | cut -d' ' -f1", returnStdout: true).trim()
